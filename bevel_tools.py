...
class BLENDTOOLS_PT_Panel_bevel(BlendTools_Panel, bpy.types.Panel):
    """Creates a Panel in the Object properties window"""
    bl_label = "Bevel"
    bl_idname = "BLENDTOOLS_PT_panel_bevel"
...
class BLENDTOOLS_PT_Panel_beveloptions(BlendTools_Panel, bpy.types.Panel):
    """Creates a Panel in the Object properties window"""
    bl_label = "Options"
    bl_idname = "BLENDTOOLS_PT_panel_beveloptions"
    bl_parent_id = BLENDTOOLS_PT_Panel_bevel.bl_idname
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        layout.prop(settings, "enum_bevelAction", expand=True)
...
class BLENDTOOLS_PT_Panel_updatebevel(BlendTools_Panel, bpy.types.Panel):
    bl_label = "Update Bevel"
    bl_idname = "BLENDTOOLS_PT_panel_updatebevel"
    ...
    def draw(self, context):
        ...
        settings = scene.bevelSettings
        layout = self.layout
        bevelMods = list(filter(lambda x: x.name.startswith(
            "WN_Bevel_"), bpy.context.object.modifiers))
        row = layout.row()
...
class BLENDTOOLS_OT_Operator_updatebevel(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bevelList = list(filter(lambda x: x.name.startswith(
            "WN_Bevel_"), bpy.context.object.modifiers))
        smoothList = list(filter(lambda x: x.name.startswith(
            "WN_Smooth"), bpy.context.object.modifiers))
        ...
        for target in targets["selected_objects"]:
            ...
            if target.type == "MESH":
                bevelList = list(
                    filter(lambda x: x.name.startswith("WN_Bevel_"), target.modifiers))
                if len(bevelList) > 0:
                    bevelToUpdate = bevelList[-1]
                else:
                    if smoothAmount == 0:
                        bpy.ops.object.shade_smooth()
                        smoothMod = bpy.ops.object.modifier_add_node_group(
                            asset_library_type='ESSENTIALS', asset_library_identifier="", relative_asset_identifier="nodes\\geometry_nodes_essentials.blend\\NodeTree\\Smooth by Angle")
...
def add_smooth_modifier(ob, angle):
    bpy.ops.object.shade_smooth()
    ...
    for mod in reversed(ob.modifiers):
        if mod.type == "NODES" and "Smooth" in mod.name:
            mod.name = f"WN_SMOOTH"
            mod["Input_1"] = angle
            return mod


class BLENDTOOLS_OT_Operator_smooth(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            for ob in obs:
                ...
                if ob.type == "MESH":
                    smoothList = list(
                        filter(lambda x: x.name.startswith("WN_Smooth"), ob.modifiers))
                    smoothAmount = len(smoothList)
                    if smoothAmount == 0:
                        bpy.ops.object.shade_smooth()
                        bpy.ops.object.modifier_add_node_group(asset_library_type='ESSENTIALS', asset_library_identifier="",
...
class BLENDTOOLS_OT_Operator_weighted(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            for ob in obs:
                ...
                if ob.type == "MESH":
                    smoothList = list(filter(lambda x: x.name.startswith(
                        "WN_Weighted Normal"), ob.modifiers))
                    smoothAmount = len(smoothList)
                    if smoothAmount == 0:
...
class BLENDTOOLS_OT_Operator_bevel(bpy.types.Operator):
            ...
            for ob in obs:
                ...
                if ob.type == "MESH":
                    ...
                    bpy.ops.mesh.edges_select_sharp(
                        sharpness=BevelSettings.float_sharpAngle)
                    bpy.ops.transform.edge_bevelweight(value=1)
...
class BLENDTOOLS_OT_Operator_bevelmodonly(bpy.types.Operator):

    bl_idname = "object.bevelmodonly"
    bl_label = "bevelmodonly"
    bl_description = "Add Bevel Modifier"
